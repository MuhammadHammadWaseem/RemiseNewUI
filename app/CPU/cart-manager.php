...
public static function add_to_cart($request, $from_api = false)
{
    ...
    // If the user is not logged in, show a "Login First" message
    if ($user == 'offline') {
        return [
            'status' => 0,
            'message' => translate('Please login first to add items to the cart!')
        ];
    }
    $product = Product::find($request->id);
    $type_id = 'customer_id';
    ...
    // Remove the out-of-stock check
    // Commented the below condition to allow purchasing out-of-stock items
    // if (($product['product_type'] == 'physical') && ($product['current_stock'] < $request['quantity'])) {
    //     return [
    //         'status' => 0,
    //         'message' => translate('out_of_stock!')
    ...
    // Check the string and set the price for the variant
    if ($str != null) {
        $count = count(json_decode($product->variation));
        for ($i = 0; $i < $count; $i++) {
            if (json_decode($product->variation)[$i]->type == $str) {
                $price = json_decode($product->variation)[$i]->price;
                // Commented out the out-of-stock quantity check
                // if (json_decode($product->variation)[$i]->qty < $request['quantity']) {
                //     return [
                //         'status' => 0,
                //         'message' => translate('out_of_stock!')
 ...
 public static function update_cart_qty($request)
{
    ...
    // Remove stock validation for variations
    $count = count(json_decode($product->variation));
    if ($count) {
        for ($i = 0; $i < $count; $i++) {
            if (json_decode($product->variation)[$i]->type == $cart['variant']) {
                // Remove stock comparison here
            }
        }
    }

    // Remove stock validation for physical products
    // else if (($product['product_type'] == 'physical') && $product['current_stock'] < $request->quantity) {
    //     $status = 0;
    //     $qty = $cart['quantity'];
    // }

    // Always update quantity and shipping cost
