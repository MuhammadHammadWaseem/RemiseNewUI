...
class WebController extends Controller
{
    ...
    //                     'as-api-key' => 'asat_78a9f2f6fbeb4e42b14183dd26bc8dfd',
...
public function checkout_courier()
{
    ...
    foreach ($cart_group_ids as $group_id) {
        // Get cart details along with seller and product information
        $carts = Cart::with(['seller', 'shop', 'product'])->where('cart_group_id', $group_id)->get();
        ...
        foreach ($carts as $cart) {
            if ($cart->product_type == 'physical') {
                ...
                // Fetch the shipping address from session
                $shippingAddress = ShippingAddress::findOrFail(session('address_id'));

                // Simulate shipping service details (This could be based on your own logic)
                $services[] = [
                    'service_name' => 'Standard Shipping',  // Shipping service name
                    'delivery_time' => '3-5 business days', // Estimated delivery time
                    'cost' => 10.00                         // Shipping cost (set your own logic)
                ];

                // Create the order for each cart item and assign it to the $order variable
                $order = Order::create([
                    'customer_id' => auth()->id(),
                    'shipping_cost' => 10.00,
                ...
                // Now store the details in order_detail using the $order->id
                OrderDetail::create([
                    'order_id' => $order->id,   // Associate with the order
                    'product_id' => $cart->product_id,
                    'qty' => $cart->quantity,
                    'price' => $cart->price,    // Store price here
                    'tax' => 0,                 // Set tax logic here
                    'discount' => 0,            // Set discount logic here
                ]);
            }
        }
    }

    // Return the checkout view with simulated shipping services
    return view('web-views.checkout-payment', compact('services'));
}
    ...
    public function shop_cart(Request $request)
    {
        ...
        // }
        if(auth('seller')->check() && Cart::where(['customer_id' => auth('seller')->id()])->count() > 0){
            return view('web-views.shop-cart');
        }
        Toastr::info(translate('no_items_in_basket'));
        return redirect('/');
...
public function nonSellerRequest(Request $request){
     //$smtpMailAddress = config('mail.from.address');
     $smtpMailAddress = "hafiz.hanif@gmail.com";
     $recipientEmail = 'hafiz.hanif@gmail.com';
    $mailData = 'nothing goes here 2';
     $userData = auth('seller')->user();
    ...
    $approvalData = "First Name: ".$userData->f_name." - Last Name: ".$userData->l_name." - Phone: ".$userData->phone." - Email: ".$userData->email." - Status: ".$userData->status;
    //  Mail::send('emailVerification', $approvalData, function ($message) use ($smtpMailAddress) {
    //     $message->to($smtpMailAddress)
    //         ->subject('Approval Request');
    // });
    
    //return Mail::to($smtpMailAddress)->send(new \App\Mail\TestEmailSender());
    // Mail::to($recipientEmail)->send(new \App\Mail\PasswordResetMail($mailData));
    
    $ticket = [
            'subject' => 'Seller Account Approval',
            'type' => 'Seller Approval',
            'customer_id' => auth('customer')->check() ? auth('customer')->id() : auth('seller')->id(),
            'priority' => 'High',
            'description' => $approvalData,
            'created_at' => now(),
            'updated_at' => now(),
        ];
        DB::table('support_tickets')->insert($ticket);
    
    Toastr::success(translate('Your request has been submitted'));
    return redirect()->route('home');
    
}
public function nonSeller()
    {
        $brand_setting = BusinessSetting::where('type', 'product_brand')->first()->value;
        $home_categories = Category::where('home_status', true)->priority()->get();
