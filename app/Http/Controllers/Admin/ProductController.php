...
class ProductController extends BaseController
{
    ...
    {
        $product = Product::with(['reviews','seller.shop'])->where(['id' => $id])->first();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('brand_id') && $request->brand_id == 'other' && empty($request->other_brand)) {
        // Add a custom validation error if "other" is selected but no new brand name is provided
        $validator->after(function ($validator) {
            $validator->errors()->add('other_brand', 'Please provide a new brand name!');
        });
        }
        ...
        $p->hand_orientation = $request->hand_orientation;
        $p->added_by = "admin";
    ...
    if ($request->file('images')) {
        foreach ($request->file('images') as $index => $image) {
             $path = 'product/';
            if (in_array($index+1, $request->input('is_meta', []))) {
                $meta_images = ImageManager::upload($path . 'meta/', 'png', $image);
            } elseif (in_array($index+1, $request->input('is_thumbnail', []))) {
                $thumbnail_image = ImageManager::upload($path . 'thumbnail/', 'png', $image);
            } else {
                $product_images[] = ImageManager::upload($path, 'png', $image);
            }
        }
    }

    $p->images = json_encode($product_images); // Save all images
    $p->meta_image = $meta_images; // Save only selected meta images
    $p->thumbnail = $thumbnail_image; // Save selected thumbnail image
        // $product_images = [];
        ...
        $p->brand_id             = isset($request->brand_id) ? $request->brand_id : null;
    ...
    function list(Request $request, $type)
    {
        ...
        if ($type == 'in_house') {
            ...
            $pro = Product::with('seller.shop')->where(['added_by' => 'seller'])->where('request_status', $request->status);
        }
    ...
    // }
        ...
        public function export_excel(Request $request, $type) {
            $products = Product::query()
                ->when($type == 'in_house', function ($q) {
                    // For admin products
                    $q->where('added_by', 'seller');
                })
                ->when($type != 'in_house', function ($q) use ($request) {
                    // For seller products based on request_status
                    if ($request->has('status')) {
                        $q->where('added_by', 'seller')
                          ->where('request_status', $request->status);
                    } else {
                        $q->where('added_by', 'seller');
                    }
                })
                // Handle admin seeing both admin and seller products
                ->when(auth()->check() && auth()->user()->role == 'admin', function ($q) {
                    // If admin, include both admin and seller products
            ...
            if ($products->isEmpty()) {
                \Log::info('No products found for type: ' . $type . ' and status: ' . $request->status);
                return back()->withErrors('No products found to export.');
            }
    ...
    {    
        ...
        if ($request->has('hand_orientation_checkbox')) {
            $product->hand_orientation = $request->input('hand_orientation');  // Save the selected hand
        } else {
            $product->hand_orientation = null;  // If unchecked, set hand_orientation to null
        }
        $choice_options = [];
        if ($request->has('choice')) {
            foreach ($request->choice_no as $key => $no) {
        ...
        $product->discount = $request->discount ? ($request->discount_type == 'flat' ? Convert::usd($request->discount) : $request->discount) : 0;
        ...
        if ($request->ajax()) {
                ...
                foreach ($request->file('images') as $index => $image) {
                    ...
                    if (in_array($index+1, $request->input('is_meta', []))) {
                        ...
                        if ($product->meta_image) {
                            ImageManager::delete($path . 'meta/' . $product->meta_image);
                        }
                        $meta_image = ImageManager::upload($path . 'meta/', 'png', $image);
                    } elseif (in_array($index+1, $request->input('is_thumbnail', []))) {
                        // Delete old thumbnail if exists
                        if ($product->thumbnail) {
                            ImageManager::delete($path . 'thumbnail/' . $product->thumbnail);
                        }
            ...
            $product->meta_image = $meta_image ?? $product->meta_image;
            $product->thumbnail = $thumbnail_image ?? $product->thumbnail;
            // $product->images = json_encode($product_images);
    ...
    public function remove_thumbnail_image(Request $request)
    {
        ImageManager::delete('/product/thumbnail' . $request->name);
        $product = Product::find($request['id']);
    ...
    public function remove_meta_image(Request $request)
    {
        ImageManager::delete('/product/meta' . $request->name);
        $product = Product::find($request['id']);
