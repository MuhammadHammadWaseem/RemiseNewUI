...
class SellerController extends Controller
{
    ...
    {
        // Fetch sellers with their products and orders
        $sellers = Seller::with(['shop', 'product', 'orders'])->get();
        ...
        $data = $sellers->map(function ($seller) {
            return [
                ...
                'First Name' => $seller->f_name,
                'Last Name' => $seller->l_name,
                'Email' => $seller->email,
                'Phone' => $seller->phone,
                'Status' => $seller->status,
                'Product Count' => $seller->product->count(), // Count of products
                'Order Count' => $seller->orders->where('seller_is', 'seller')->where('order_type', 'default_type')->count(), // Count of orders
            ];
        });
    
        // Use FastExcel to download the file
        return (new FastExcel($data))->download('sellers_list.xlsx');
    }
       ...
       public function deleteSeller(Request $request)
        {
            ...
            Seller::destroy($ids);
            return response()->json(['success' => true, 'message' => 'Sellers deleted successfully!']);
        }
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($request->status == "approved") {
            ...
            Shop::where('seller_id',$request->id)->delete();
            Order::where('seller_id',$request->id)->delete();
            SellerWallet::where('seller_id',$request->id)->delete();
            Toastr::info('Seller has been deleted successfully');
            return redirect('admin/sellers/seller-list');
        }
        $order->save();
        return back();
    }
