...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->has('other_brand') && !empty($request->other_brand)){
            $brand = new Brand;
            $brand->name = $request->other_brand ?? null;
            $brand->status = 1;
            $brand->save();
    ...
    //         // $product_images[] = $image_name; // Save all images for the 'images' column

    //         // Check for meta image
    //         if (in_array($index, $request->input('is_meta', []))) {
    //             $meta_images = ImageManager::upload('product/meta/', 'png', $img);
    //         }else if (in_array($index+1, $request->input('is_thumbnail', []))) {
    //             $thumbnail_image = ImageManager::upload('product/thumbnail/', 'png', $img);
    //         }else{
    //             $image_name = ImageManager::upload('product/', 'png', $img);
    //             $product_images[] = $image_name; // Save all images for the 'images' column
    //         }
    ...
    // $product->images = json_encode($product_images); // Save all images
    // $product->meta_image = $meta_images; // Save only selected meta images
    // $product->thumbnail = $thumbnail_image; // Save selected thumbnail image
    ...
    $product->thumbnail = $thumbnail_image;
