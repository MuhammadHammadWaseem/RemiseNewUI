...
class LoginController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        } elseif (isset($se) && $se['status'] == 'pending' && auth('seller')->attempt(['email' => $request->email, 'password' => $request->password], $request->remember)) {
             Toastr::info('Welcome to Remise Fencing!');
            if (SellerWallet::where('seller_id', auth('seller')->id())->first() == false) {
                DB::table('seller_wallets')->insert([
                    'seller_id' => auth('seller')->id(),
                    'withdrawn' => 0,
