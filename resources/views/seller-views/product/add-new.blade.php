...
@push('css_or_js')
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}">
        <style>
.card-body .input-checkbox-align {
    display: flex;
...
.card-body .input-checkbox-align input {
    width: 20px;
    height: 20px;
}

.card-body .input-checkbox-align label {
    width: fit-content;
    margin: 0;
}


div#coba .form-group.mt-2 {
...
div#coba .form-group.mt-2 label {
    display: flex;
    align-items: center;
    gap: 5px;
    margin: 0;
...
div#coba .form-group.mt-2:last-child {
    border: none;
}
.align-two-checkboxes {
    display: flex;
    align-items: center;
        ...
        <div class="row">
            <div class="col-md-12">
                <form class="product-form" action="{{ route('seller.product.add-new') }}" method="post"
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <!--    </select>-->
                                    <!--</div>-->
                                    <!--this code add after comment the above code-->
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        <label for="name" class="title-color">{{ \App\CPU\translate('Brand') }}</label>
                                        <select id="brand_select" class="js-example-basic-multiple js-states js-example-responsive form-control"
                                        name="brand_id" required>
                                            <option value="{{ null }}" selected disabled>---{{ \App\CPU\translate('Select') }}---</option>
                                                @foreach ($br as $b)
                                                    <option value="{{ $b['id'] }}">{{ $b['name'] }}</option>
                                                @endforeach
                                             <option value="other">{{ \App\CPU\translate('Other') }}</option> <!-- Added 'Other' option -->
                                        </select>
                                    </div>

                                    <!-- Hidden input for manually entering the brand when 'Other' is selected -->
                                    <div id="other_brand_input" class="col-md-4 mb-3" style="display: none;">
                                        <label for="other_brand" class="title-color">{{ \App\CPU\translate('Enter Brand Name') }}</label>
                                        <input type="text" id="other_brand" name="other_brand" class="form-control" placeholder="{{ \App\CPU\translate('Brand Name') }}">
                                    </div>
                                    ...
                                    <div class="col-md-4 mb-3">
                                         ...
                                         <div class="input-checkbox-align">
                                        <label for="hand_orientation_checkbox" class="title-color">{{ \App\CPU\translate('Select Hand Orientation') }}</label>
                                        <input class="form-control" type="checkbox" id="hand_orientation_checkbox" name="hand_orientation_checkbox">
                                        </div>
                                        
                                        
                                        <div class="check-box-displayed" id="hand_orientation_dropdown" style="display: none;">
                                            <!--<label for="hand_orientation" class="title-color">Hand Orientation</label>-->
                                            <select id="hand_orientation" name="hand_orientation" class="form-control">
                                                <option value="" selected>{{ \App\CPU\translate('Select Hand') }}</option>
                                                <option value="RH">{{ \App\CPU\translate('Right Hand (RH)') }}</option>
                                                <option value="LH">{{ \App\CPU\translate('Left Hand (LH)') }}</option>
                                            </select>
                                        </div>
                                    ...
                                    <!-- Hand Orientation Dropdown (Initially Hidden) -->
                                    <!--<div class="col-md-4 mb-3" id="hand_orientation_dropdown" style="display: none;">-->
                                    <!--    <label for="hand_orientation" class="title-color">Hand Orientation</label>-->
                                    <!--    <select id="hand_orientation" name="hand_orientation" class="form-control">-->
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <div class="row align-items-end">
                                    ...
                                    <input type="hidden" name="tax_model" value="include">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="hidden" min="1" value="1" step="1"
                    ...
                    <!--                <input type="hidden" value="fencing gaer" name="meta_title" placeholder="" class="form-control">-->
                    <!--            </div>-->
                    ...
                    <!--                <input type="hidden" name="meta_description" value="Used gear for the sport of fencing for purchase or sale">-->
                    <!--            </div>-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--</div>-->
...
@push('script')
        ...
        $(function() {
            ...
            $("#coba").spartanMultiImagePicker({
                ...
                onAddRow: function(index, file) {
                    // Adding input fields and checkboxes after a new image is added
                    let newRow = $('#coba').find('.spartan_item_wrapper').last(); // Select the last added image
                    let inputFields = `<div class="align-two-checkboxes">
                        <div class="form-group mt-2">
                            <label>
                                <input type="checkbox" name="is_meta[]" value="${index}"> Meta
                            </label>
                        </div>
                        <div class="form-group mt-2">
                            <label>
                                <input type="checkbox" name="is_thumbnail[]" value="${index}"> Thumbnail
                            </label>
                        </div> </div> `;
                    newRow.append(inputFields); // Append input fields and checkboxes to the new image container
                },
                onRenderedPreview: function(index) {
                    // Find the row associated with the current index
                    let newRow = $('#coba').find(`.spartan_item_wrapper[data-spartanindexrow="${index}"]`);
                    
                    // Check if the current index is 0 (first image)
                    if (index == 0) {
                        // Find the "Thumbnail" checkbox and check it
                        newRow.find('input[name="is_thumbnail[]"]').prop('checked', true);
                    }
                },
    ...
    <script>
        ...
        $('#brand_select').on('change', function() {
            ...
            var otherBrandInput = document.getElementById('other_brand_input');
            if(this.value == "other"){
         ...
         $('#hand_orientation_checkbox').change(function() {
            if ($(this).is(':checked')) {
                // Show the dropdown when checkbox is checked
                $('#hand_orientation_dropdown').show();
            } else {
                // Hide the dropdown when checkbox is unchecked
                $('#hand_orientation_dropdown').hide();
                // Optionally clear the selected value when hiding the dropdown
